...
--
-- Host: 127.0.0.1
-- Generation Time: Oct 07, 2015 at 09:37 PM
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `productview` AS (select `p`.`product_id` AS `product_id`,`p`.`product_name` AS `product_name`,`b`.`brand_Name` AS `brand_Name`,`c`.`name` AS `name`,`c`.`parent_id` AS `parent_id`,`p`.`brand_id` AS `brand_id`,`p`.`category_id` AS `category_id` from ((`product` `p` join `category` `c` on((`p`.`category_id` = `c`.`id`))) join `brand` `b` on((`p`.`brand_id` = `b`.`id`))));
